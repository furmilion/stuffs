...
from math import tau, sin, floor, ceil
...
class Channel:
    ...
    def update(self, suppress_phase_update=False, suppress_noise_update=False):
        ...
        # TODO: глобальные волны чтобы класс не заставлял комп лететь на тяое кулеровв
        # от полной оперативы
...
def get_note(note = 60, tune = 440): # for some tests
    return (2 ** ((-57 + note) / 12)) * tune

def key_from_note(note = "c-5"):
    dat = list(note.lower().split("-"))
    dat[1] = int(dat[1])
    notes = {"c": 0, "c#": 1,
             "d": 2, "d#": 3,
             "e": 4,
...
if __name__ == "__main__":  # main loop where i test stuff; envelopes
    ...
    ChannelNoise.set_attack(0); ChannelNoise.set_decay1(.15); ChannelNoise.set_sustain(0); ChannelNoise.set_decay2(-1); ChannelNoise2.set_release(0)
    ChannelNoise2.set_attack(0);ChannelNoise2.set_decay1(.15);ChannelNoise2.set_sustain(0);ChannelNoise2.set_decay2(-1);ChannelNoise2.set_release(0)
    ...
    k = key_from_note
    notes = [
        f(k("e-3"),450), f(k("a-3"),450), f(k("b-3"),450), f(k("g-4"),450),
        f(k("g-3"),450), f(k("b-3"),450), f(k("d-4"),450), f(k("a-4"),450),
        f(k("e-3"),450), f(k("b-3"),450), f(k("a-4"),450), f(k("b-4"),450),
        f(k("e-4"),450), f(k("b-4"),450), f(k("a-4"),450), f(k("d-5"),450),
        f(k("e-3"),450), f(k("a-3"),450), f(k("b-3"),450), f(k("g-4"),450),
        f(k("g-3"),450), f(k("b-3"),450), f(k("d-4"),450), f(k("a-4"),450),
        f(k("e-3"),450), f(k("b-3"),450), f(k("a-4"),450), f(k("b-4"),450),
        f(k("e-4"),450), f(k("d-5"),450), f(k("e-5"),450), f(k("b-4"),450),
        ...
        f(k("e-3"),450), f(k("a-3"),450), f(k("b-3"),450), f(k("g-4"),450),
        f(k("g-3"),450), f(k("b-3"),450), f(k("d-4"),450), f(k("a-4"),450),
        f(k("e-3"),450), f(k("b-3"),450), f(k("f#-4"),450), f(k("a-4"),450),
        f(k("d-5"),450), f(k("f#-5"),450), f(k("d-5"),450), f(k("e-5"),450),
    ]
    gates = [ 1,]
    vols =  [ .1,]
    seq = 0
    ...
    for _ in range(floor(sr*length)):
        ...
        #if not _ % (sr//10):  # gate at roughly 120 beats, every 16th note
            if not gates[seq % len(gates)]:
                pass
            else:
                ChannelNoise.press_channel()
                ChannelNoise2.press_channel()
            ChannelNoise._freq = notes[seq] - (notes[seq]/128)
            ChannelNoise.volume = vols[seq % len(vols)] * 2
            ChannelNoise2._freq = notes[seq]
            ChannelNoise2.volume = vols[seq % len(vols)] * 2
